...
router.post('/update', async (req, res) => {
    try {
        const { projectId, slugline, sceneIndex, data } = req.body;
        ...
        memoryService_1.default.migrateProjectScenes(projectId);
        const updatedScene = memoryService_1.default.updateSceneMemory(projectId, slugline, data, sceneIndex);
        const response = {
            success: true,
            data: updatedScene
        };
...
router.get('/by-slugline', async (req, res) => {
    try {
        ...
        memoryService_1.default.migrateProjectScenes(projectId);
        const index = sceneIndex ? parseInt(sceneIndex, 10) : undefined;
        const scene = memoryService_1.default.getSceneBySlugline(projectId, slugline, index);
        const response = {
            success: true,
            data: scene
        };
...
router.get('/by-id', async (req, res) => {
    try {
        ...
        const scene = memoryService_1.default.getSceneById(projectId, sceneId);
        const response = {
            success: true,
            data: scene
        };
...
router.delete('/scene', async (req, res) => {
    try {
        ...
        const index = sceneIndex ? parseInt(sceneIndex, 10) : undefined;
        const deleted = memoryService_1.default.deleteScene(projectId, slugline, index);
        if (!deleted) {
            const response = {
                success: false,
