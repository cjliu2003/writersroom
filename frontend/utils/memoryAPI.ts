...
export class MemoryAPI {
  ...
  /**
   * Update or add a scene in memory with optional scene index
   */
  ...
  /**
   * Get a scene by slugline with optional scene index
   */
  static async getSceneBySlugline(
    projectId: string,
    slugline: string,
    sceneIndex?: number
  ): Promise<SceneMemory | undefined> {
    let url = `/memory/by-slugline?projectId=${encodeURIComponent(projectId)}&slugline=${encodeURIComponent(slugline)}`;
    if (sceneIndex !== undefined) {
      url += `&sceneIndex=${sceneIndex}`;
    }

    const response = await apiRequest<SingleSceneResponse>(url);
    return response.data;
  }

  /**
   * Get a scene by its composite ID
   */
  static async getSceneById(projectId: string, sceneId: string): Promise<SceneMemory | undefined> {
    const response = await apiRequest<SingleSceneResponse>(
      `/memory/by-id?projectId=${encodeURIComponent(projectId)}&sceneId=${encodeURIComponent(sceneId)}`
    );
  ...
  /**
   * Delete a specific scene with optional scene index
   */
  static async deleteScene(
    ...
    let url = `/memory/scene?projectId=${encodeURIComponent(projectId)}&slugline=${encodeURIComponent(slugline)}`;
    if (sceneIndex !== undefined) {
      url += `&sceneIndex=${sceneIndex}`;
    }

    await apiRequest<SceneMemoryResponse>(url, { method: 'DELETE' });
  }
...
export const useMemoryAPI = (projectId: string) => {
  const updateScene = async (
    ...
    return MemoryAPI.updateSceneMemory(projectId, slugline, data, sceneIndex);
  };
  ...
  return {
    ...
    deleteScene: (slugline: string, sceneIndex?: number) =>
      MemoryAPI.deleteScene(projectId, slugline, sceneIndex),
    getSceneById: (sceneId: string) => MemoryAPI.getSceneById(projectId, sceneId),
  };
};
