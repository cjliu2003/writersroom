...
export default function EditorPage() {
  ...
  const extractTitleFromScenes = (memoryScenes: MemoryScene[]): string | null => {
    ...
    if (memoryScenes.length > 0) {
      const firstSlugline = memoryScenes[0].slugline
  ...
  const convertMemoryScenesToScript = (memoryScenes: MemoryScene[]): string => {
    ...
    memoryScenes.forEach((scene, index) => {
      // Add scene heading
      scriptContent += scene.slugline + '\n\n'
      
      // Add scene summary as action
      if (scene.summary) {
        scriptContent += scene.summary + '\n\n'
      }
      
      // Add characters as dialogue placeholders if available
      if (scene.characters && scene.characters.length > 0) {
        scene.characters.forEach(character => {
          scriptContent += `${character.toUpperCase()}\n`
          scriptContent += `(dialogue)\n\n`
        })
      }
      
      // Add spacing between scenes
      if (index < memoryScenes.length - 1) {
        scriptContent += '\n'
      }
    })
