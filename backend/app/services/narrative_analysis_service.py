...
Scene numbers must be 1-{total_scenes}. setup_scene must be < payoff_scene."""
    ...
    def _parse_threads_response(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            data = json.loads(json_str.strip())
            return data.get("plot_threads", [])

        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse plot threads JSON: {e}")
            return []
    ...
    def _parse_relationships_response(self, response: str) -> List[Dict]:
        ...
        try:
            ...
            data = json.loads(json_str.strip())
            return data.get("scene_relationships", [])

        except json.JSONDecodeError as e:
            logger.error(f"Failed to parse scene relationships JSON: {e}")
            return []
