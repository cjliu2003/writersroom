...
from app.routers import health_router, auth_router, script_router, fdx_router, user_router, ai_router, ai_ingestion_router, ai_analytics_router, scene_autosave_router, script_autosave_router, script_websocket
...
app.include_router(ai_analytics_router.router, prefix="/api")
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        print(f"✅ Completed: {request.method} {request.url.path} - {response.status_code} - {process_time:.3f}s")
        return response
    except Exception as e:
        process_time = time.time() - start_time
        print(f"❌ Failed: {request.method} {request.url.path} - {type(e).__name__}: {str(e)} - {process_time:.3f}s")
        raise
